...
"""
    WENO_step!(u::Tuple{Vararg{Array{<:Real, 2}}},
               v::NamedTuple{(:x, :y), <:Tuple{Vararg{AbstractArray{<:Real}, 2}}},
               weno::WENOScheme,
               Δt, Δx, Δy;
...
"""
function WENO_step!(u::Tuple{Vararg{Array{<:Real, 2}}}, v::NamedTuple{(:x, :y), <:Tuple{Vararg{AbstractArray{<:Real}, 2}}}, weno::WENOScheme, Δt, Δx, Δy; u_min::Tuple{Vararg{Real}}, u_max::Tuple{Vararg{Real}})
    for i in eachindex(u)
        WENO_step!(u[i], v, weno, Δt, Δx, Δy; u_min = u_min[i], u_max = u_max[i])
    end
    return nothing
end
