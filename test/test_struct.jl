@testset "struct tests" begin
    @testset "1D" begin
        u0 = ones(3)
        weno = WENOScheme(u0; form = :nonconservative, stag = false)
...
@testset "multiphase struct tests" begin
    ...
    @testset "1D" begin
        ...
        # vcenter (the prepared cell-centred velocity) exists only on the staggered path
        @test scheme.vcenter === nothing
        staggered = MultiphaseWENOScheme(phases; stag = true)
        @test staggered.vcenter isa NamedTuple{(:x,), <:Tuple{Vector{Float64}}}
        @test size(staggered.vcenter.x) == (6,)

        # The constructed object is fully concrete, which is what the hot loop needs.
        # The constructor's own return type is a two-way Union over `vcenter` whenever
        # `stag` is a runtime value, because `vcenter` is a `NamedTuple` of arrays when
        # staggered and `nothing` when collocated. That Union is resolved once, outside
        # any loop.
    ...
    @testset "2D and 3D shapes" begin
        ...
        @test size(s2.vcenter.x) == (3, 4)
        @test size(s2.vcenter.y) == (3, 4)
