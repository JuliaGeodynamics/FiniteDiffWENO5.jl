@testset "3D limiter tests" begin
    @testset "3D linear case" begin
        ...
        u = copy(u0)
        weno = WENOScheme(u; boundary = (2, 2, 2, 2, 2, 2), stag = false, lim_ZS = true, multithreading = true)
        ...
        while t < tmax
            WENO_step!(u, v, weno, Δt, Δx, Δy, Δz; u_min = 0.0, u_max = 1.0)
        ...
        @test maximum(u) ≈ 0.9429094498386863 atol = 1.0e-8
    end
    ...
    @testset "3D linear case KA CPU" begin
        ...
        copyto!(u, u0)
        weno = WENOScheme(u, backend; boundary = (2, 2, 2, 2, 2, 2), stag = false, lim_ZS = true)
        ...
        while t < tmax
            WENO_step!(u, v, weno, Δt, Δx, Δy, Δz, backend; u_min = 0.0, u_max = 1.0)
        ...
        @test isapprox(sum(u), sum(u0); atol = 1.0e-6)
        @test maximum(u) ≈ 0.9425373300176934 atol = 1.0e-8
    end
    ...
    @testset "3D linear case Chmy CPU" begin
        ...
        set!(u, u0)
        weno = WENOScheme(u, grid; boundary = (2, 2, 2, 2, 2, 2), stag = false, lim_ZS = true, multithreading = true)
        ...
        while t < tmax
            WENO_step!(u, v, weno, Δt, Δx, Δy, Δz, grid, arch; u_min = 0.0, u_max = 1.0)
