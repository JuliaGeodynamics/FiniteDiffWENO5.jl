@kernel inbounds = true function multiphase_WENO_flux_KA_2D_x!(
    ...
    sl = ntuple(
        q -> (
            state[q][iwww, j], state[q][iww, j], state[q][iw, j],
            state[q][ie, j], state[q][iee, j],
        ), phase_count
    )
    sr = ntuple(
        q -> (
            state[q][iww, j], state[q][iw, j], state[q][ie, j],
            state[q][iee, j], state[q][ieee, j],
        ), phase_count
    )
    up = limit_simplex(
...
@kernel inbounds = true function multiphase_WENO_flux_KA_2D_y!(
    ...
    sl = ntuple(
        q -> (
            state[q][i, jwww], state[q][i, jww], state[q][i, jw],
            state[q][i, je], state[q][i, jee],
        ), phase_count
    )
    sr = ntuple(
        q -> (
            state[q][i, jww], state[q][i, jw], state[q][i, je],
            state[q][i, jee], state[q][i, jeee],
        ), phase_count
    )
    up = limit_simplex(
        multiphase_reconstruction_upwind(sl, χ, γ, ζ, ϵ),
